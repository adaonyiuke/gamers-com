-- Fix fun stats logic:
-- 1. Marathon Gamer: require >= 2 games in one night (avoid showing 1-game nights)
-- 2. Beginner's Luck: find the most recent session where someone won their first
--    game, return ALL qualifying players from that session as a players array
...
BEGIN
  ...
  -- Requires >= 2 games to avoid showing trivial 1-game results
  ...
  -- ── Fun Stats: Beginner's Luck ─────────────────────────────────────
  -- Find each member's very first finalized session
  first_game_per_member AS (
    SELECT DISTINCT ON (member_id)
           member_id, session_id, is_winner, game_name, finalized_at
    FROM member_entries
    ORDER BY member_id, finalized_at ASC
  ),
  -- All members who won their debut game
  lucky_first_timers_all AS (
    SELECT fg.member_id, fg.session_id, fg.game_name, fg.finalized_at,
           gm.display_name, gm.avatar_url
    FROM first_game_per_member fg
    JOIN group_members gm ON gm.id = fg.member_id
    WHERE fg.is_winner = true
  ),
  -- The most recent session in which beginner's luck occurred
  latest_lucky_session AS (
    SELECT session_id, game_name
    FROM lucky_first_timers_all
    ORDER BY finalized_at DESC
    LIMIT 1
  ),
  -- All winners from that session (may be multiple)
  lucky_first_timer AS (
    SELECT lft.display_name, lft.avatar_url, lls.game_name
    FROM lucky_first_timers_all lft
    JOIN latest_lucky_session lls USING (session_id)
  )
